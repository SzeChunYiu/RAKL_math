...
def test_rh_ana_002_strict_packet_passes_current_pre_candidate_gates() -> None:
    ...
    assert set(memory_raw["selected_tool_ids"]) == {
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                ...
                "Li coefficients",
                "finite-place residual S_f(n)",
                "zero-free regions",
            ),
            relations=(
                "arithmetic explicit formula",
                "incomplete Li coefficient through height sqrt(n)",
                "finite coefficient information to partial zero exclusion",
                "all-index positivity to RH",
            ),
            domain="analytic number theory / Riemann Hypothesis",
            goal_type="classify zeta-specific intermediate Li/prime-side targets by implication strength",
        ),
...
def test_rh_ana_002_source_packet_keeps_route_strength_and_regularization_boundaries() -> None:
    text = (
        BASE / "01_frontier/RH_ANA_002_PRIMARY_SOURCE_PACKET_20260811.md"
    ).read_text(encoding="utf-8")
    assert "root-strength" in text
    assert "partial zero-free target" in text
    assert "regularized" in text
