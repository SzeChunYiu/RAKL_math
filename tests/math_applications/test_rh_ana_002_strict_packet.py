...
def test_rh_ana_002_li_norm_defect_packet_passes_pre_candidate_gates() -> None:
    context_raw = _load("01_frontier/RH_ANA_002_CONTEXT_FIBER_20260811.json")
    ...
    assert context_raw["atom_id"] == "RH-ANA-002"
    ...
    tools_raw = _load("07_memory/RH_ANA_002_TOOL_SNAPSHOT_20260811.json")
    failures_raw = _load("07_memory/RH_ANA_002_FAILURE_SNAPSHOT_20260811.json")
    memory_raw = _load("07_memory/RH_ANA_002_RESEARCH_MEMORY_REVIEW_20260811.json")
    assert {tool["tool_id"] for tool in tools_raw["tools"]} == {
        "T-RH-LI-PREFIX-QUARTET-CALIBRATION",
        "T-XM-ROOT-BRIDGE-STABILITY-AUDIT",
    }
    assert {failure["failure_id"] for failure in failures_raw["experiences"]} == {
        "F-RH-ANA-001-FINITE-LI-PREFIX",
        "F-XM001-POINTWISE-GAP-COLLAPSE",
    }
    ...
    assert memory_raw["selected_tool_ids"] == ["T-XM-ROOT-BRIDGE-STABILITY-AUDIT"]
    assert any("DifferenceWitness" in note for note in memory_raw["tool_applicability_notes"])
    ...
    trace_raw = _load("09_trace/RH_ANA_002_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                ...
                "Li coefficients lambda_n",
                "Suzuki functions G_n",
                "Weil explicit-formula functional",
                "prime/archimedean arithmetic representation",
            ),
            relations=(
                "Li all-index positivity",
                "unconditional L2 membership",
                "RH-equivalent norm identity",
                "explicit-formula faithfulness",
                "all-index growth and cancellation control",
            ),
            domain="analytic number theory / Riemann Hypothesis",
            goal_type="localize a noncircular zeta-specific faithfulness bridge from positive analytic surrogates to all-index Li positivity",
        ),
    ...
    assert trace.entries[-1].outputs == (
        "next_action:LI_NORM_FAITHFULNESS_MATRIX",
