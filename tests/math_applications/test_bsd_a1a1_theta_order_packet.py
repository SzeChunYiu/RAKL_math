...
def test_bsd_a1a1_theta_order_packet_passes_pre_candidate_gates() -> None:
    context_raw = _load("01_frontier/BSD_A1a1_THETA_ORDER_CONTEXT_FIBER_20260811.json")
    ...
    fiber = MathContextFiber(atom_id=context_raw["atom_id"], object_context=context_raw["object_context"], structural_coordinates=tuple(context_raw["structural_coordinates"]), equivalent_formulations=tuple(context_raw["equivalent_formulations"]), solved_analogues=tuple(context_raw.get("solved_analogues", ())), near_solved_analogues=tuple(context_raw.get("near_solved_analogues", ())), method_transfers=tuple(MethodTransfer(source_context=item["source_context"], method=item["method"], shared_structure=tuple(item["shared_structure"]), required_assumptions=tuple(item["required_assumptions"]), disanalogies=tuple(item["disanalogies"]), repair_question=item["repair_question"], source_anchors=tuple(item["source_anchors"])) for item in context_raw["method_transfers"]), explicit_disanalogies=tuple(context_raw["explicit_disanalogies"]), source_anchors=tuple(context_raw["source_anchors"]), analogy_scan_status=context_raw["analogy_scan_status"], cross_domain_analogies=tuple(CrossDomainAnalogy(source_kind=item["source_kind"], source_situation=item["source_situation"], common_abstraction=tuple(item["common_abstraction"]), source_to_target_mapping=tuple(item["source_to_target_mapping"]), shared_constraints=tuple(item["shared_constraints"]), disanalogies=tuple(item["disanalogies"]), proposed_principle=item["proposed_principle"], validation_obligation=item["validation_obligation"], provenance_note=item["provenance_note"]) for item in context_raw["cross_domain_analogies"]), analogy_scan_notes=context_raw["analogy_scan_notes"], frozen_at=context_raw["frozen_at"], first_candidate_at=context_raw["first_candidate_at"], packet_hash=context_raw["packet_hash"])
    assert audit_math_context_fiber(fiber).verdict is ContextGateVerdict.PASS
    tools_raw = _load("07_memory/BSD_A1a1_RESEARCH_TOOL_INVENTORY_20260811.json")
    failures_raw = _load("07_memory/BSD_A1a1_FAILURE_EXPERIENCE_LATTICE_20260811.json")
    memory_raw = _load("07_memory/BSD_A1a1_RESEARCH_MEMORY_REVIEW_20260811.json")
    ...
    memory = ResearchMemoryReview(target_atom_id=memory_raw["target_atom_id"], target_context_hash=memory_raw["target_context_hash"], tool_inventory_snapshot_hash=memory_raw["tool_inventory_snapshot_hash"], failure_lattice_snapshot_hash=memory_raw["failure_lattice_snapshot_hash"], tool_query_status=MemoryQueryStatus(memory_raw["tool_query_status"]), failure_query_status=MemoryQueryStatus(memory_raw["failure_query_status"]), candidate_method_families=tuple(memory_raw["candidate_method_families"]), relevant_tool_ids=tuple(memory_raw["relevant_tool_ids"]), relevant_failure_ids=tuple(memory_raw["relevant_failure_ids"]), selected_tool_ids=tuple(memory_raw["selected_tool_ids"]), tool_applicability_notes=tuple(memory_raw["tool_applicability_notes"]), failure_reuse_notes=tuple(memory_raw["failure_reuse_notes"]), unresolved_warnings=tuple(memory_raw["unresolved_warnings"]), evidence_pointers=tuple(memory_raw["evidence_pointers"]), artifact_hash=memory_raw["artifact_hash"])
    ...
    trace_raw = _load("09_trace/BSD_A1a1_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""; entries = []
    for raw in trace_raw["entries"]:
        assert raw["previous_event_hash"] == previous
        payload = copy.deepcopy(raw); artifact_hash = payload["artifact_hash"]; payload["artifact_hash"] = ""
