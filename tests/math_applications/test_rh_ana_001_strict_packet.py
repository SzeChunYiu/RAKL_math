...
def test_rh_ana_001_first_strict_analytic_packet_passes_all_pre_candidate_gates() -> None:
    context_raw = _load("01_frontier/RH_ANA_001_CONTEXT_FIBER_20260811.json")
    ...
    tools_raw = _load("07_memory/RH_ANA_001_RESEARCH_TOOL_INVENTORY_20260811.json")
    failures_raw = _load("07_memory/RH_ANA_001_FAILURE_EXPERIENCE_LATTICE_20260811.json")
    memory_raw = _load("07_memory/RH_ANA_001_RESEARCH_MEMORY_REVIEW_20260811.json")

    assert tools_raw == {"tools": []}
    assert failures_raw == {"experiences": [], "links": []}
    assert memory_raw["tool_query_status"] == "NO_RELEVANT_MATCH"
    assert memory_raw["failure_query_status"] == "NO_RELEVANT_MATCH"
    ...
    trace_raw = _load("09_trace/RH_ANA_001_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                ...
                "admissible analytic test functions",
            ),
            relations=(
                "Mellin transform",
                "prime-zero explicit formula",
                "multiplicative autocorrelation",
                "RH-equivalent sign criterion",
            ),
            domain="analytic number theory / Riemann Hypothesis",
            goal_type="localize a candidate-independent RH analytic obstruction",
        ),
