...
def test_hm1a_strict_packet_passes_current_pre_candidate_gates() -> None:
    context_raw = _load("01_frontier/HM1a_CONTEXT_FIBER_20260811.json")
    ...
    tools_raw = _load("07_memory/HM1a_RESEARCH_TOOL_INVENTORY_20260811.json")
    failures_raw = _load("07_memory/HM1a_FAILURE_EXPERIENCE_LATTICE_20260811.json")
    ...
    trace_raw = _load("09_trace/HM1a_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            ...
            relations=(
                ...
                "source-bearing assembly",
            ),
            domain="complex algebraic geometry / Hodge theory / motives",
            goal_type="audit a higher-codimension source-bearing decomposition",
        ),
...
def test_hm1a_post_calibration_deltas_are_schema_valid_if_present() -> None:
    tool_delta = BASE / "07_memory/HM1a_RESEARCH_TOOL_DELTA_20260811.json"
    failure_delta = BASE / "07_memory/HM1a_FAILURE_EXPERIENCE_DELTA_20260811.json"
    if not (tool_delta.exists() and failure_delta.exists()):
        return

    tool = json.loads(tool_delta.read_text(encoding="utf-8"))["tool"]
    failure = json.loads(failure_delta.read_text(encoding="utf-8"))["experience"]
