...
def test_yms1_first_packet_passes_all_pre_candidate_gates_without_candidate() -> None:
    context_raw = _load("01_frontier/YM-S1_CONTEXT_FIBER_20260811.json")
    ...
    tools_raw = _load("07_memory/YM-S1_RESEARCH_TOOL_INVENTORY_20260811.json")
    failures_raw = _load("07_memory/YM-S1_FAILURE_EXPERIENCE_LATTICE_20260811.json")
    memory_raw = _load("07_memory/YM-S1_RESEARCH_MEMORY_REVIEW_20260811.json")
    ...
    trace_raw = _load("09_trace/YM-S1_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                "reflection-positive lattice Yang-Mills transfer matrix",
                "gauge-invariant source family",
                "finite-volume spectral/correlation gap",
                "renormalization-group trajectory",
                "continuum reconstructed Hamiltonian",
            ),
            relations=(
                "Euclidean-time evolution / spectral measure",
                "thermodynamic limit",
                "RG/coarse-graining transport",
                "lattice-spacing scaling",
                "OS/Wightman reconstruction",
            ),
            domain="lattice gauge theory / operator theory / constructive Yang-Mills",
            goal_type=(
                "transport finite-lattice spectral information to a positive physical "
