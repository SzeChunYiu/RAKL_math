...
def test_rh_spec_002a_strict_packet_passes_before_norm_gap_calibration() -> None:
    ...
    assert any("|Im z|<1/2" in item for item in context_raw["structural_coordinates"])
    fiber = _fiber(context_raw)
    assert audit_math_context_fiber(fiber).verdict is ContextGateVerdict.PASS

    tools_raw = _load("07_memory/RH_SPEC_002_POSTCAL_RESEARCH_TOOL_INVENTORY_20260811.json")
    failures_raw = _load("07_memory/RH_SPEC_002_POSTCAL_FAILURE_EXPERIENCE_LATTICE_20260811.json")
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                "semilocal Weil-form ground state xi_lambda",
                "prolate-derived proxy k_lambda",
                "source-defined scalar normalization c_lambda",
                "complex Fourier/Mellin transforms on |Im z|<1/2",
            ),
            relations=(
                "ground-state/proxy approximation",
                "weighted transform continuity",
                "lowest-eigenspace stability",
                "spectral separation or source-specific rigidity",
                "domain-local Hurwitz zero transport",
            ),
            domain="analytic number theory / operator theory / prolate harmonic analysis / complex analysis",
            goal_type="calibrate the norm-and-gap bridge before any source-specific RH theorem candidate",
        ),
