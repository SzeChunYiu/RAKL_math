...
def test_rh_spec_002a_strict_packet_passes_before_gap_residual_calibration() -> None:
    context_raw = _load("01_frontier/RH_SPEC_002A_CONTEXT_FIBER_20260811.json")
    ...
    memory_raw = _load("07_memory/RH_SPEC_002A_RESEARCH_MEMORY_REVIEW_20260811.json")
    tool_snapshot = BASE / "07_memory/RH_SPEC_002_POSTCAL_RESEARCH_TOOL_INVENTORY_20260811.json"
    failure_snapshot = BASE / "07_memory/RH_SPEC_002_POSTCAL_FAILURE_EXPERIENCE_LATTICE_20260811.json"
    assert memory_raw["tool_inventory_snapshot_hash"] == _git_blob_hash(tool_snapshot)
    assert memory_raw["failure_lattice_snapshot_hash"] == _git_blob_hash(failure_snapshot)
    ...
    assert memory_raw["selected_tool_ids"] == [
        "T-RH-SPEC-HURWITZ-DOMAIN-ZERO-TRANSPORT"
    ]
    ...
    trace_raw = _load("09_trace/RH_SPEC_002A_OPEN_TRACE_20260811.json")
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                "Weil quadratic-form ground state xi_lambda",
                "finite truncations xi_(lambda,N)",
                "prolate proxy k_lambda",
                "source-normalized Fourier/regularized determinant",
                "Riemann Xi function",
            ),
            relations=(
                "ground-state spectral separation",
                "proxy residual or Rayleigh excess",
                "fixed-lambda truncation convergence",
                "weighted source-space to compact-uniform transform convergence",
                "cofinal N/lambda limit discipline",
            ),
            domain="analytic number theory / self-adjoint operator theory / prolate spectral analysis",
            goal_type="freeze a gap-residual discriminator before any source-side RH spectral theorem candidate",
        ),
