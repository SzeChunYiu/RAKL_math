...
def test_bsd_s001_first_strict_packet_passes_all_pre_candidate_gates() -> None:
    context_raw = _load("01_frontier/BSD-S001_CONTEXT_FIBER_20260811.json")
    ...
    memory_raw = _load("07_memory/BSD-S001_RESEARCH_MEMORY_REVIEW_20260811.json")
    ...
    trace_raw = _load("09_trace/BSD-S001_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                "elliptic curve E/Q",
                "p-infinity Selmer group",
                "complex L-function",
                "refined BSD leading-term data",
            ),
            relations=(
                "Selmer exact sequence",
                "Iwasawa main-conjecture comparison",
                "explicit reciprocity",
                "complex-to-p-adic leading-term bridge",
            ),
            domain="arithmetic geometry / Iwasawa theory",
            goal_type="identify a higher-rank analytic-arithmetic BSD bridge",
        ),
