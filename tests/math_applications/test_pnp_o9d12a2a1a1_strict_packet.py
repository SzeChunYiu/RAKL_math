...
def test_o9d12a2a1a1_source_closure_pre_candidate_packet_passes_current_gates() -> None:
    context_raw = _load("01_frontier/O9d12a2a1a1_CONTEXT_FIBER_20260811.json")
    ...
    tools_raw = _load("07_memory/O9d12a2a1a1_RESEARCH_TOOL_INVENTORY_20260811.json")
    failures_raw = _load("07_memory/O9d12a2a1a1_FAILURE_EXPERIENCE_LATTICE_20260811.json")
    memory_raw = _load("07_memory/O9d12a2a1a1_RESEARCH_MEMORY_REVIEW_20260811.json")

    for tool in tools_raw["tools"]:
        payload = copy.deepcopy(tool)
        payload["artifact_hash"] = ""
        assert tool["artifact_hash"] == _canonical_hash(payload)
    for failure in failures_raw["experiences"]:
        payload = copy.deepcopy(failure)
        payload["artifact_hash"] = ""
        assert failure["artifact_hash"] == _canonical_hash(payload)
    ...
    trace_raw = _load("09_trace/O9d12a2a1a1_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=("cover pair family", "source-native minimal preservation closure G_w", "derived intersection activations"),
            relations=("semi-filter preservation", "upward closure", "pair-preservation implication", "cyclic fusion reuse"),
            domain="complexity theory / graph cover complexity / finite closure systems",
            goal_type="audit a higher-order noncanonical representation before invariant invention",
        ),
