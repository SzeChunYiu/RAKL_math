...
def test_bsd_a1_first_strict_packet_passes_all_pre_candidate_gates() -> None:
    context_raw = _load("01_frontier/BSD_A1_RANK2_CONTEXT_FIBER_20260811.json")
    ...
    memory_raw = _load("07_memory/BSD_A1_RESEARCH_MEMORY_REVIEW_20260811.json")

    assert tools_raw["tools"] == []
    assert failures_raw["experiences"] == []
    assert failures_raw["links"] == []
    ...
    trace_raw = _load("09_trace/BSD_A1_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            ...
            relations=(
                ...
                "height pairing",
                "Selmer descent",
                "exterior/determinantal independence",
            ),
            domain="arithmetic geometry / analytic number theory",
            goal_type="isolate the rank-two BSD analytic-arithmetic bridge",
        ),
