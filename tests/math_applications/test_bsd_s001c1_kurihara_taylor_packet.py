...
def test_bsd_s001c1_kurihara_taylor_packet_passes_pre_candidate_gates() -> None:
    context_raw = _load(
        "01_frontier/BSD-S001c1_KURIHARA_TAYLOR_CONTEXT_FIBER_20260811.json"
    )
    ...
    memory_raw = _load("07_memory/BSD-S001c1_RESEARCH_MEMORY_REVIEW_20260811.json")
    ...
    trace_raw = _load("09_trace/BSD-S001c1_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                ...
                "Kurihara-number collection",
                "Kato Kolyvagin system",
                "p-primary Bloch-Kato Selmer group",
            ),
            relations=(
                "complex Taylor order",
                "discrete Kurihara order",
                "explicit reciprocity",
                "localized Iwasawa main conjecture",
                "Selmer corank comparison",
            ),
            domain="arithmetic geometry / Iwasawa theory",
            goal_type="identify a non-circular arbitrary-rank discrete-to-complex BSD order bridge",
        ),
