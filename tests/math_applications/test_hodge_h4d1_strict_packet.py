...
def test_h4d1_strict_pre_candidate_packet_passes_current_rakl_gates() -> None:
    context_raw = _load("01_frontier/H4d1_CONTEXT_FIBER_20260811.json")
    ...
    memory_raw = _load("07_memory/H4d1_RESEARCH_MEMORY_REVIEW_20260811.json")
    ...
    trace_raw = _load("09_trace/H4d1_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                ...
                "Hodge-locus branch",
                "algebraic-cycle witness space",
            ),
            relations=(
                "parallel transport",
                "cycle class realization",
                "witness projection",
                "deformation obstruction",
                "formal algebraization",
            ),
            domain="algebraic geometry / Hodge theory / deformation theory",
            goal_type="discover a deformation-lifting criterion for algebraic cycles",
        ),
