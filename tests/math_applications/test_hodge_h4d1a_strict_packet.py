...
def test_h4d1a_strict_pre_candidate_packet_passes_current_rakl_gates() -> None:
    context_raw = _load("01_frontier/H4d1a_CONTEXT_FIBER_20260811.json")
    ...
    memory_raw = _load("07_memory/H4d1a_RESEARCH_MEMORY_REVIEW_20260811.json")
    ...
    trace_raw = _load("09_trace/H4d1a_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                ...
                "flat rational Hodge class",
                "Hodge-locus branch",
                "chosen algebraic-cycle witness",
                "branch-reachable obstruction envelope",
                "semiregularity detector",
            ),
            relations=(
                "parallel transport",
                "witness deformation",
            ...
            domain="algebraic geometry / Hodge theory / deformation and obstruction theory",
            goal_type="calibrate a strict-weaker branch-restricted witness-lifting mechanism",
        ),
...
def test_h4d1a_packet_keeps_scope_and_hostile_controls_explicit() -> None:
    review = (BASE / "08_reviews/H4d1a_EXPERT_CONTEXT_REVIEW_20260811.md").read_text(
        encoding="utf-8"
    )
    addendum = (BASE / "01_frontier/H4d1a_SOURCE_ADDENDUM_20260811.md").read_text(
        encoding="utf-8"
    )
