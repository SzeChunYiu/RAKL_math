...
def test_yme1a1_packet_passes_current_pre_candidate_gates_without_candidate() -> None:
    context_raw = _load("01_frontier/YM_E1a1_CONTEXT_FIBER_20260811.json")
    ...
    fiber = MathContextFiber(
        ...
        method_transfers=tuple(MethodTransfer(source_context=i["source_context"], method=i["method"], shared_structure=tuple(i["shared_structure"]), required_assumptions=tuple(i["required_assumptions"]), disanalogies=tuple(i["disanalogies"]), repair_question=i["repair_question"], source_anchors=tuple(i["source_anchors"])) for i in context_raw["method_transfers"]),
        ...
        cross_domain_analogies=tuple(CrossDomainAnalogy(source_kind=i["source_kind"], source_situation=i["source_situation"], common_abstraction=tuple(i["common_abstraction"]), source_to_target_mapping=tuple(i["source_to_target_mapping"]), shared_constraints=tuple(i["shared_constraints"]), disanalogies=tuple(i["disanalogies"]), proposed_principle=i["proposed_principle"], validation_obligation=i["validation_obligation"], provenance_note=i["provenance_note"]) for i in context_raw.get("cross_domain_analogies", ())),
        analogy_scan_notes=context_raw.get("analogy_scan_notes", ""),
        frozen_at=context_raw["frozen_at"], first_candidate_at=context_raw.get("first_candidate_at"), packet_hash=context_raw["packet_hash"])
    assert audit_math_context_fiber(fiber).verdict is ContextGateVerdict.PASS

    tools_raw = _load("07_memory/YM_E1a1_RESEARCH_TOOL_INVENTORY_20260811.json")
    failures_raw = _load("07_memory/YM_E1a1_FAILURE_EXPERIENCE_LATTICE_20260811.json")
    memory_raw = _load("07_memory/YM_E1a1_RESEARCH_MEMORY_REVIEW_20260811.json")
    ...
    assert memory_raw["relevant_failure_ids"] == ["F-YM-S1A-RESTRICTED-SOURCE-HIDDEN-STATE"]
    copied_failure = copy.deepcopy(failures_raw["experiences"][0])
    failure_hash = copied_failure["artifact_hash"]
    copied_failure["artifact_hash"] = ""
    assert failure_hash == _canonical_hash(copied_failure)
    ...
    trace_raw = _load("09_trace/YM_E1a1_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
