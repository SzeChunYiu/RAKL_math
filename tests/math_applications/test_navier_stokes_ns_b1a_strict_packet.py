...
def test_ns_b1a_child_packet_passes_current_strict_pre_candidate_gates() -> None:
    context_raw = _load("01_frontier/NS-B1a_CONTEXT_FIBER_20260811.json")
    ...
    memory_raw = _load("07_memory/NS-B1a_RESEARCH_MEMORY_REVIEW_20260811.json")
    ...
    trace_raw = _load("09_trace/NS-B1a_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                "mild bounded ancient Navier-Stokes solution",
                "finite Type-I parabolic control",
                "global spatial tail / backward L3 trace",
            ),
            relations=(
                "local-to-global transport",
                "pressure coupling",
                "ancient mild evolution",
                "Liouville activation",
            ),
            domain="partial differential equations / fluid mechanics",
            goal_type="isolate dynamics-specific Type-I trace/tail bridge",
        ),
...
def test_ns_b1_sparse_tail_calibration_is_fail_closed_in_scope() -> None:
