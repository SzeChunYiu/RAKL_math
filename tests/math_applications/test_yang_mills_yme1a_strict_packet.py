...
def test_yme1a_first_packet_passes_all_pre_candidate_gates_without_candidate() -> None:
    context_raw = _load("01_frontier/YM_E1a_CONTEXT_FIBER_20260811.json")
    ...
    tools_raw = _load("07_memory/YM_E1a_RESEARCH_TOOL_INVENTORY_20260811.json")
    failures_raw = _load("07_memory/YM_E1a_FAILURE_EXPERIENCE_LATTICE_20260811.json")
    memory_raw = _load("07_memory/YM_E1a_RESEARCH_MEMORY_REVIEW_20260811.json")
    ...
    trace_raw = _load("09_trace/YM_E1a_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                "pure four-dimensional Yang-Mills regulator",
                "renormalized gauge-invariant observable algebra",
                "continuum Schwinger-function limit",
            ),
            relations=(
                "lattice refinement / cutoff removal",
                "renormalization-group transport",
                "reflection positivity",
                "gauge invariance",
            ),
            domain="constructive quantum field theory / Yang-Mills theory",
            goal_type="construct a non-trivial OS-compatible continuum observable theory",
        ),
