...
def test_bsd_s001c1a_complex_upper_bound_packet_is_strict_pre_candidate() -> None:
    context_raw = _load(
        "01_frontier/BSD-S001c1a_COMPLEX_UPPER_BOUND_CONTEXT_FIBER_20260811.json"
    )
    ...
    tools_raw = _load("07_memory/BSD-S001_RESEARCH_TOOL_INVENTORY_20260811.json")
    failures_raw = _load("07_memory/BSD-S001_FAILURE_EXPERIENCE_LATTICE_20260811.json")
    memory_raw = _load("07_memory/BSD-S001c1a_RESEARCH_MEMORY_REVIEW_20260811.json")
    ...
    trace_raw = _load("09_trace/BSD-S001c1a_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                "elliptic curve E/Q",
                "complex Hasse-Weil L-function",
                "normalized discrete or Mazur-Tate order",
                "Kato/Kurihara arithmetic data",
            ),
            relations=(
                "complex Taylor order",
                "augmentation/discrete order",
                "correction normalization",
                "one-sided upper bound",
                "Selmer/Sha interpretation",
            ),
            domain="arithmetic geometry / Iwasawa theory",
            goal_type="isolate a non-circular complex-order upper bound or representation obstruction",
        ),
...
def test_bsd_s001c1_bounded_source_audit_keeps_root_authority_closed() -> None:
    audit = (
        BASE / "01_frontier/BSD-S001c1_BOUNDED_SOURCE_COMPARISON_20260811.md"
    ...
    assert "literature-wide nonexistence claim" in audit
