...
def test_ns_r001_first_strict_packet_passes_pre_candidate_gates() -> None:
    context_raw = _load("01_frontier/NS_R001_CONTEXT_FIBER_20260811.json")
    ...
    failures_raw = _load("07_memory/NS_R001_FAILURE_EXPERIENCE_LATTICE_20260811.json")
    memory_raw = _load("07_memory/NS_R001_RESEARCH_MEMORY_REVIEW_20260811.json")
    ...
    trace_raw = _load("09_trace/NS_R001_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    research_trace = MathResearchTrace(
        trace_id=trace_raw["trace_id"], entries=tuple(entries)
    )
    assert (
        audit_pre_candidate_trace(
            research_trace,
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                ...
                "scale-critical regularity quantity",
            ...
            goal_type="derive or falsify a global critical-control mechanism",
        ),
