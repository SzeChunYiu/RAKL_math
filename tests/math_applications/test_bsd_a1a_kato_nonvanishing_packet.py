...
def test_bsd_a1a_kato_nonvanishing_packet_passes_pre_candidate_gates() -> None:
    context_raw = _load(
        "01_frontier/BSD_A1a_KATO_NONVANISHING_CONTEXT_FIBER_20260811.json"
    )
    ...
    tools_raw = _load("07_memory/BSD_A1_RESEARCH_TOOL_INVENTORY_20260811.json")
    failures_raw = _load("07_memory/BSD_A1_FAILURE_EXPERIENCE_LATTICE_20260811.json")
    memory_raw = _load("07_memory/BSD_A1a_RESEARCH_MEMORY_REVIEW_20260811.json")
    ...
    trace_raw = _load("09_trace/BSD_A1a_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=(
                "complex elliptic-curve L-function",
                "generalised Kato class",
                "p-adic Selmer group",
                "localization map",
                "p-adic leading term",
            ),
            relations=(
                "complex central order two",
                "explicit reciprocity",
                "class nonvanishing",
                "Selmer dimension",
                "localization",
            ),
            domain="arithmetic geometry / analytic number theory / Iwasawa theory",
            goal_type="isolate the complex analytic-rank-two to Kato nonvanishing bridge",
        ),
