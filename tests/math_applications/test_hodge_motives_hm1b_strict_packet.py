...
def test_hm1b_strict_packet_passes_current_pre_candidate_gates() -> None:
    context_raw = _load("01_frontier/HM1b_CONTEXT_FIBER_20260811.json")
    ...
    tools_raw = _load("07_memory/HM1b_RESEARCH_TOOL_INVENTORY_20260811.json")
    failures_raw = _load("07_memory/HM1b_FAILURE_EXPERIENCE_LATTICE_20260811.json")
    memory_raw = _load("07_memory/HM1b_RESEARCH_MEMORY_REVIEW_20260811.json")

    jsonschema.Draft202012Validator(
        json.loads((FRAMEWORK_ROOT / "schemas/research-tool-inventory.schema.json").read_text()),
        format_checker=jsonschema.FormatChecker(),
    ).validate(tools_raw)
    jsonschema.Draft202012Validator(
        json.loads((FRAMEWORK_ROOT / "schemas/failure-experience-lattice.schema.json").read_text()),
        format_checker=jsonschema.FormatChecker(),
    ...
    trace_raw = _load("09_trace/HM1b_PRE_CANDIDATE_TRACE_20260811.json")
    previous = ""
    entries = []
    for raw in trace_raw["entries"]:
        ...
        entries.append(
            ResearchTraceEntry(
                ...
                outputs=tuple(raw.get("outputs", ())), uncertainties=tuple(raw.get("uncertainties", ())),
                residuals=tuple(raw.get("residuals", ())), next_steps=tuple(raw.get("next_steps", ())),
                artifact_hash=raw["artifact_hash"], previous_event_hash=raw.get("previous_event_hash", ""),
            )
        )
    ...
    plan = plan_math_research(
        signature=ProblemSignature(
            objects=("smooth projective complex variety", "rational Hodge class", "algebraic correspondence generator", "source algebraic class", "Hodge-empty complement"),
            relations=("cycle class map", "correspondence composition", "source-span coverage", "Hodge decomposition"),
            domain="complex algebraic geometry / Hodge theory / motives",
            goal_type="audit algebraic-generator source coverage before theorem invention",
        ),
