...
**Atom:** `YM-S1a1`  
**Candidate ID:** `YM-S1A1-C001-V2-DENSE-SOURCE-COMMON-RATE-SPECTRAL-EXCLUSION`  
**Supersedes:** `YM-S1A1-C001-DENSE-SOURCE-COMMON-RATE-SPECTRAL-EXCLUSION` only for the target-facing `q=0` boundary clarification; the v1 bytes remain immutable.  
...
The v2 statement deliberately requires `q>0`. The abstract spectral-projection argument also has a valid extended endpoint at `q=0`, but the ordinary real-valued Hamiltonian expression `-log q` does not. The target-facing candidate therefore excludes that endpoint rather than relying on an unstated extended-real convention.
...
The frozen v1 candidate allowed `q=0` and then wrote `-(1/a) log q` without specifying an extended-real convention. Hostile exact-head review correctly treated that as a blocking boundary defect. This successor narrows the registered candidate to `0<q<1`; it does not alter the spectral proof for positive `q`, the three registered worlds, or any Yang–Mills target obligation. The correction supplies no independent-review, novelty, formal-proof, or root authority.
